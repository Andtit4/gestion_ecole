...
// API de secours pour générer un PDF de bulletin sans authentification
export async function POST(req: NextRequest) {
  try {
    console.log('POST /api/debug/pdf - Début de la requête');
    
    // Extraire l'ID du bulletin des données de la requête
    ...
    console.log(`Génération du PDF pour le bulletin ID: ${reportCardId}`);
    
    // Récupérer les données du bulletin
    let reportCard = await prisma.reportcard.findUnique({
    ...
    // Si le bulletin n'existe pas, renvoyer une erreur
    if (!reportCard) {
      return NextResponse.json({
        success: false,
    ...
    return NextResponse.json({
      ...
      pdfData: pdfBase64,
      fileName: `Bulletin_${reportCard.student.user.lastName.replace(/\s+/g, '_')}_${reportCard.student.user.firstName.replace(/\s+/g, '_')}.pdf`
    });
  } catch (error) {
    console.error('Erreur POST /api/debug/pdf:', error);
